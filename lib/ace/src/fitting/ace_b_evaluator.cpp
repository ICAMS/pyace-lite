...
ACEBEvaluator::compute_atom(int i, DOUBLE_TYPE **x, const SPECIES_TYPE *type, const int jnum, const int *jlist) {
    ...
    basis_projections_rank1.init(total_basis_size_rank1, ndensity, "b_projections_rank1");
    basis_projections.init(total_basis_size, ndensity, "b_projections");
#endif
...
//        ndensity = func->ndensity;
        ...
        for (DENSITY_TYPE p = 0; p < ndensity; ++p) {
            ...
            basis_projections_rank1(func_rank1_ind, p)+=  A_cur;
#endif
        }
    } // end loop for rank=1
...
#ifdef EXTRA_C_PROJECTIONS
                //aggregate C-projections separately
                basis_projections(func_ind, p)+=B.real_part_product(func->gen_cgs[ms_ind]);
#endif
